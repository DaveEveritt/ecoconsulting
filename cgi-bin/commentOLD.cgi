...
if ($q->param("submit")) {										# Comment submitted?
	...
	if (grep {$_->{name} eq $vals{name} && $_->{comment} eq $vals{comment}} @comments) {
		$vals{message} .= "You have already posted this comment and it is awaiting approval";	# Trap duplicates
	}
	unless ($vals{message}) {											# No errors?
		write_comment();												# Write it
		%vals=('message'=>"Thanks for taking the time to comment - it will appear after approval."); # And say thanks (whilst blanking rest of vals)
	}
}
...
[ENDCOMMENTS]
		...
		<p>Comments (limited to 2000 characters or 300 words) will appear after approval. Your email <em>will not be published</em>, but include it if you want a personal response.</p>
